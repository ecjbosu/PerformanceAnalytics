###############################################################################
# $Id: VaR.R 3301 2014-01-18 15:26:12Z braverock $
###############################################################################
...
#' @param reasoncheck TRUE/FALSE whether to conduct reasonableness checks.  False to
...
function (R=NULL , p=0.95, method=c("modified","gaussian","historical", "kernel"), 
         ...
         weights=NULL, mu=NULL, sigma=NULL, m3=NULL, m4=NULL, invert=TRUE, reasoncheck=FALSE, ...)
...
# Joe added flag off 7/17/14
        ...
        marginal = {
          # check if weightsFlag exists
#          dots3 <- list(...);
#          idx <- names(dots3)=="weightsFlag";
          weightsFlag <- ifelse (hasArg(weightsFlag), weightsFlag, T);
    	    return(VaR.Marginal(R,p,method,as.vector(weights), mu, sigma, m3, m4, invert, reasoncheck, weightsFlag))

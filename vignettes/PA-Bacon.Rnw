...
\author {
\begin{tabular}{cc}
		{Original Author} & {Matthieu Lestel}\\
		\small {Edited by} & \small {Joe Wayne Byers}
\end{tabular} 
}
...
To calculate Mean absolute deviation we take the sum of the absolute value of the difference between the returns and the mean of the returns and we divide it by the number of returns.
 ...
 \deqn{MeanAbsoluteDeviation = \frac{\sum^{n}_{i=1}r_i - \overline{r}}{n}}\\
...
where \eqn{n}. is the number of observations of the entire series, \eqn{d}. is number of drawdowns, \eqn{r_P}. is the portfolio return, \eqn{r_F}. is the risk free rate and \eqn{D_t}. the \eqn{t^{th}}. drawdown.  The denominator in the modified Burke ratio is the Drawdown Deviation.
...
where \eqn{r_P}. is the annualized portfolio return, \eqn{r_F}. is the risk free rate, \eqn{n}. is the number of observations of the entire series, \eqn{D'_i}. is the drawdown since previous peak in period \eqn{i}.  The denominator in the Martin ration is the Ulcer Index.
...
where \eqn{r_P}. is the annualized portfolio return, \eqn{r_F}. is the risk free rate, \eqn{n}. is the number of observations of the entire series, \eqn{D'_i}. is the drawdown since previous peak in period \eqn{i}.  The denominator in the Pain ratio is the Pain Index.

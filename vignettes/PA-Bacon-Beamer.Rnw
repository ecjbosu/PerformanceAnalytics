...
% \begin{frame}
% $M^2$ is a risk adjusted return useful to judge the size of relative performance between differents portfolios. With it you can compare portfolios with different levels of risk.
% 
% \begin{equation*}
...
% \begin{frame}
% $M^2$ excess is the quantity above the standard M. There is a geometric excess return which is better for Bacon and an arithmetic excess return.
% 
% \begin{equation*}
...
% 
% \begin{equation*}
% {DownsideDeviation(R , MAR) = \delta_{MAR} = \sqrt{\sum^{n}_{t=1}\frac{min[(R_{t} - MAR), 0]^2}{n\end{equation*}
% 
% }}}
...
% \end{equation*}
% 
% where $\delta_{MAR}$ is the $\code{\link{DownsideDeviation}$.
% 
% <<>>=
% data(managers)
